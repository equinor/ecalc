...
def create_electrical_topology(energy_unit_factory_factory, demand: ElectricalPower):
    source = energy_unit_factory_factory(ElectricalSource, "source")
    consumer = ElectricalConsumer("consumer", demand=demand)
...
class TestEnergyNetworkEnergyCalculation:
    def test_calculates_input_and_output_energy_through_network(
        ...
        pump = MechanicalConsumer("pump", demand=MechanicalPower(4))
        base_load = ElectricalConsumer("base_load", demand=ElectricalPower(5))
        ...
        energy_network = energy_network_simulation.run(
            consumer_demands={
                pump.get_id(): pump.get_demand(),
                base_load.get_id(): base_load.get_demand(),
            }
        )
    ...
    def test_returns_no_connection_energy_for_source_without_successors(
        ...
        energy_network = energy_network_simulation.run(consumer_demands={})
    ...
    def test_calculates_input_energy_for_transporter(
        ...
        consumer = ElectricalConsumer("consumer", demand=ElectricalPower(10))
        ...
        energy_network = energy_network_simulation.run(consumer_demands={consumer.get_id(): consumer.get_demand()})
...
class TestEnergyNetworkInputValidation:
    def test_requires_demand_for_every_consumer(self, energy_network_simulation_factory, energy_unit_factory_factory):
        topology, source, _ = create_electrical_topology(
            energy_unit_factory_factory,
            demand=ElectricalPower(5),
        )
        simulation = energy_network_simulation_factory(
            junctions=[],
            energy_unit_factories=[source],
            topology=topology,
        )

        with pytest.raises(InvalidEnergyNetworkInputError, match="Missing consumer demands for nodes"):
            simulation.run(consumer_demands={})

    def test_rejects_demand_for_non_consumer(self, energy_network_simulation_factory, energy_unit_factory_factory):
        ...
        with pytest.raises(InvalidEnergyNetworkInputError, match="Unexpected consumer demands for nodes"):
            simulation.run(
                consumer_demands={
                    consumer.get_id(): consumer.get_demand(),
                    source.get_id(): ElectricalPower(5),
                }
            )

    def test_rejects_demand_with_wrong_energy_type(
        self, energy_network_simulation_factory, energy_unit_factory_factory
    ):
        topology, source, consumer = create_electrical_topology(
            energy_unit_factory_factory,
        ...
        with pytest.raises(InvalidEnergyNetworkInputError, match="requires ElectricalPower, got MechanicalPower"):
            simulation.run(consumer_demands={consumer.get_id(): MechanicalPower(5)})
...
class TestEnergyNetworkCapacity:
    def test_reports_capacity_exceeded_without_capping_connection_energy(
        ...
        load = ElectricalConsumer("load", demand=ElectricalPower(6))
        topology = create_topology(
            nodes=[grid, load],
        ...
        energy_network = energy_network_simulation.run(
            consumer_demands={load.get_id(): load.get_demand()},
    ...
    def test_capacity_equal_to_connection_energy_has_no_failure(
        ...
        grid = energy_unit_factory_factory(ElectricalSource, "grid")
        load = ElectricalConsumer("load", demand=ElectricalPower(5))
        topology = create_topology(
            nodes=[grid, load],
            connections=[(grid.get_id(), load.get_id())],
        )
    ...
    def test_missing_capacity_means_unlimited(self, energy_network_simulation_factory, energy_unit_factory_factory):
        topology, grid, load = create_electrical_topology(
            energy_unit_factory_factory,
            demand=ElectricalPower(5),
        )
        energy_network_simulation = energy_network_simulation_factory(
        ...
        energy_network = energy_network_simulation.run(consumer_demands={load.get_id(): load.get_demand()})
    ...
    def test_compares_capacity_with_total_outgoing_energy(
        ...
        grid = energy_unit_factory_factory(ElectricalSource, "grid")
        first_load = ElectricalConsumer("first_load", demand=ElectricalPower(3))
        second_load = ElectricalConsumer("second_load", demand=ElectricalPower(4))
        topology = create_topology(
        ...
        energy_network = energy_network_simulation.run(
            consumer_demands={
                first_load.get_id(): first_load.get_demand(),
                second_load.get_id(): second_load.get_demand(),
            },
            capacities=capacities,
        )
        assert energy_network.get_capacity_failures()[grid.get_id()].status == EnergyFailureStatus.CAPACITY_EXCEEDED
...
class TestJunctionDispatch:
    def test_dispatches_to_shore_before_genset(self, energy_network_simulation_factory, energy_unit_factory_factory):
        ...
        load = ElectricalConsumer("load", demand=ElectricalPower(8))
    ...
    def test_separate_genset_curves_reproduce_legacy_fuel_jump(
        ...
        load = ElectricalConsumer("load", demand=ElectricalPower(12))
    ...
    def test_dispatch_uses_cable_capacity_not_upstream_grid_capacity(
        ...
        load = ElectricalConsumer("load", demand=ElectricalPower(25))
    ...
    def test_rejects_dispatch_strategy_candidate_ids_for_junction_with_a_single_predecessor(
        ...
        load = ElectricalConsumer("load", demand=ElectricalPower(0))
        other_load = ElectricalConsumer("other_load", demand=ElectricalPower(0))
    ...
    def test_accepts_dispatch_strategy_naming_the_only_predecessor(
        ...
        first_load = ElectricalConsumer("first_load", demand=ElectricalPower(3))
        second_load = ElectricalConsumer("second_load", demand=ElectricalPower(7))
        ...
        bus_load = ElectricalConsumer("bus_load", demand=ElectricalPower(0))
        direct_load = ElectricalConsumer("direct_load", demand=ElectricalPower(0))
...
class TestEnergyNetworkState:
    ...
    def test_records_state_for_junction(self, energy_network_simulation_factory, energy_unit_factory_factory):
        grid = energy_unit_factory_factory(ElectricalSource, "grid")
        bus = ElectricalBus("bus")
        load = ElectricalConsumer("load", demand=ElectricalPower(7))
        topology = create_topology(
            nodes=[grid, bus, load],
