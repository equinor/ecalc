...
class TestJunctionDispatch:
    ...
    def test_overflows_last_candidate_when_total_capacity_is_insufficient(self):
        ...
        assert not evaluation.is_feasible(connection_energy=energy, capacities=capacities)
    ...
    def test_dispatch_uses_cable_capacity_not_upstream_grid_capacity(self):
        ...
        assert evaluation.is_capacity_exceeded(node_id=grid.get_id(), connection_energy=energy, capacities=capacities)
        assert not evaluation.is_feasible(connection_energy=energy, capacities=capacities)
