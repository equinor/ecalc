...
from libecalc.energy.models.sampled_compressor_data import PD_NAME, PS_NAME, RATE_NAME, SampledCompressorData
...
COMMON_MODELS = {
    (RATE_NAME,): _RateCompressor1D,
    (PS_NAME,): _SuctionPressureCompressor1D,
    (PD_NAME,): _DischargePressureCompressor1D,
    (RATE_NAME, PD_NAME): _RatePdCompressor2D,
    (RATE_NAME, PS_NAME): _RatePsCompressor2D,
    (PS_NAME, PD_NAME): _PsPdCompressor2D,
    (RATE_NAME, PS_NAME, PD_NAME): _SampledCompressor3D,
}
...
def _legacy_evaluate(data: dict[str, list[float]], rate, suction_pressure, discharge_pressure) -> float:
    ...
    rate_input = np.asarray([rate], dtype=np.float64) if rate is not None else cast(NDArray[np.float64], None)
...
def _legacy_direct_max_rate_1d(model) -> float:
    return float(np.asarray(model.get_max_rate(), dtype=np.float64).reshape(-1)[0])
...
def test_dispatcher_matches_legacy(data, evaluation, active_variables):
    common = SampledCompressorFactory.create(**data)

    assert isinstance(common, COMMON_MODELS[active_variables])
...
def test_1d_direct_matches_legacy(x_name, sampled_data, test_data):
    ...
    common = common_cls(
        SampledCompressorData.from_raw_lists(
            energy_usage_values=frame["ENERGY"].tolist(),
            rate_values=frame[RATE_NAME].tolist() if x_name == RATE_NAME else None,
            suction_pressure_values=frame[PS_NAME].tolist() if x_name == PS_NAME else None,
            discharge_pressure_values=frame[PD_NAME].tolist() if x_name == PD_NAME else None,
        )
    )
    argument_name = {
        RATE_NAME: "rate",
    ...
    for value in test_data:
        ...
        _assert_allclose(common.evaluate(**kwargs).energy_usage, _legacy_direct_evaluate(legacy, **kwargs))
    _assert_allclose(common.get_max_standard_rate(), _legacy_direct_max_rate_1d(legacy))
...
def test_2d_direct_matches_legacy(variables, rows, rate, suction_pressure, discharge_pressure):
    ...
    data = {
        "energy_usage_values": frame["ENERGY"].tolist(),
        "rate_values": frame[RATE_NAME].tolist() if RATE_NAME in variables else None,
        "suction_pressure_values": frame[PS_NAME].tolist() if PS_NAME in variables else None,
        "discharge_pressure_values": frame[PD_NAME].tolist() if PD_NAME in variables else None,
    }
    legacy = LEGACY_2D_MODELS[variables](frame, "ENERGY")
    common = COMMON_MODELS[variables](SampledCompressorData.from_raw_lists(**data))
    ...
    for query in _scalar_evaluation_cases(
        ...
        _assert_allclose(common.evaluate(**query).energy_usage, _legacy_evaluate(data, **query))

    if variables == (RATE_NAME, PD_NAME):
        for discharge_value in [0, 2, 4, 6, 8, 9]:
            _assert_allclose(
                common.get_max_standard_rate(discharge_pressure=discharge_value),
    ...
    if variables == (RATE_NAME, PS_NAME):
        for suction_value in [0, 2, 4, 6, 9]:
            _assert_allclose(
                common.get_max_standard_rate(suction_pressure=suction_value),
...
def test_3d_direct_matches_legacy():
    ...
    for query in _scalar_evaluation_cases(evaluations):
        _assert_allclose(common.evaluate(**query).energy_usage, _legacy_direct_evaluate(legacy, **query))
...
def test_get_max_standard_rate_2d_matches_get_max_rate():
    ...
    # so there is no legacy oracle for this dispatcher path. Compare the factory-dispatched
    # model against the equivalent concrete 2D model built directly from the same raw data.
    data = {
        "energy_usage_values": [52765, 76928, 118032, 145965, 71918, 109823, 137651, 139839],
        "rate_values": [1e6, 1e6, 1e6, 1e6, 3e6, 3e6, 3e6, 7e6],
        "suction_pressure_values": [50] * 8,
    ...
    common = SampledCompressorFactory.create(**data)
    assert isinstance(common, _RatePdCompressor2D)
    direct = _RatePdCompressor2D(SampledCompressorData.from_raw_lists(**data))
    for suction_pressure, discharge_pressure in [(50.0, 162.0), (50.0, 300.0), (50.0, 471.0)]:
        _assert_allclose(
            ...
            direct.get_max_standard_rate(suction_pressure=suction_pressure, discharge_pressure=discharge_pressure),
        )
...
def test_get_max_standard_rate_3d_matches_get_max_rate():
    ...
    common = SampledCompressorFactory.create(
        energy_usage_values=frame["ENERGY"].tolist(),
        rate_values=frame[RATE_NAME].tolist(),
        suction_pressure_values=frame[PS_NAME].tolist(),
        discharge_pressure_values=frame[PD_NAME].tolist(),
    )
    assert isinstance(common, _SampledCompressor3D)
    direct = _SampledCompressor3D(
        SampledCompressorData.from_raw_lists(
            energy_usage_values=frame["ENERGY"].tolist(),
            rate_values=frame[RATE_NAME].tolist(),
            suction_pressure_values=frame[PS_NAME].tolist(),
...
def test_missing_variable_raises() -> None:
    with pytest.raises(ProcessMissingVariableValidationException):
        SampledCompressorFactory.create(energy_usage_values=[1.0, 2.0, 3.0])


def test_mismatched_lengths_raises() -> None:
    with pytest.raises(ProcessEqualLengthValidationException):
        SampledCompressorFactory.create(energy_usage_values=[1.0, 2.0, 3.0], rate_values=[1.0, 2.0])
...
def test_negative_value_raises() -> None:
    with pytest.raises(ProcessNegativeValuesValidationException):
        SampledCompressorFactory.create(energy_usage_values=[1.0, 2.0, -3.0], rate_values=[1.0, 2.0, 3.0])


def test_negative_numeric_string_value_raises() -> None:
    with pytest.raises(ProcessNegativeValuesValidationException):
        SampledCompressorFactory.create(
            energy_usage_values=["1.0", "2.0", "-3.0"],  # type: ignore[list-item]
            rate_values=[1.0, 2.0, 3.0],
        )
...
def test_all_degenerate_variables_raises() -> None:
    with pytest.raises(IllegalStateException, match="non-degenerate"):
        SampledCompressorFactory.create(energy_usage_values=[1.0, 2.0, 3.0], rate_values=[1.0, 1.0, 1.0])
