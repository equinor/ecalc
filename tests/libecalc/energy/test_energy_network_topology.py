...
class TestEnergyNetworkTopology:
    ...
    def test_gets_ancestor_connections(self):
        source = ElectricalSource("source")
        cable = ElectricalCable("cable")
        bus = ElectricalBus("bus")
        load = ElectricalConsumer("load")
        ...
        # Only connections on the load's paths towards sources are ancestors; the other branch is excluded.
        assert topology.get_ancestor_connections(load.get_id()) == frozenset(
            {
                topology.get_connection(source.get_id(), cable.get_id()),
