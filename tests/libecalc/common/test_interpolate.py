...
class TestInterpolate:
    def test_1d_interpolator_basic(self):
        ...
        interp = setup_interpolator_1d(x, y)
    ...
    def test_1d_interpolator_out_of_bounds(self):
        ...
        x = np.array([0, 1, 2])
        y = np.array([0, 1, 4])
        interp = setup_interpolator_1d(x, y, fill_value=-1)
        assert interp(-1) == -1
        assert interp(3) == -1
    ...
    def test_nd_interpolator_basic(self):
        ...
        y = np.array([0, 0, 1, 1])
        z = np.array([0, 1, 2, 3])
        interp = setup_interpolator_n_dimensional([x, y], z)
    ...
    def test_nd_interpolator_out_of_bounds(self):
        ...
        y = np.array([0, 0, 1, 1])
        z = np.array([0, 1, 2, 3])
        interp = setup_interpolator_n_dimensional([x, y], z, fill_value=-99)
    ...
    def test_1d_interpolator_single_point(self):
        ...
        x = np.array([0])
        y = np.array([42])
        interp = setup_interpolator_1d(x, y, fill_value=0)
        assert interp(0) == 42
        assert interp(1) == 0
