    ...
    return chart_data_factory.from_curves(
        curves=[
            chart_curve_factory(
                ...
                polytropic_head_joule_per_kg=[10415.277000000002, 9845.316, 9254.754, 8308.089, 8307.108, 7605.693],
...
def test_single_speed_pump_energy_usage_and_failure_status(single_speed_pump_chart):
    pump = PumpModel(
        pump_chart=single_speed_pump_chart,
        head_margin=0.0,
    )
...
def vsd_pump_test_variable_speed_chart_curves(chart_data_factory, chart_curve_factory) -> ChartData:
    df = pd.DataFrame(
        [
            ...
            [3425, 826, 1601.7, 0.6975],
            [3425, 825, 1601.8, 0.6981],
...
def test_variable_speed_pump_pt2(vsd_pump_test_variable_speed_chart_curves, caplog):
    ...
    assert pump.evaluate_rate_ps_pd_density(
        ...
        discharge_pressures=np.asarray([61]),
        fluid_densities=fluid_densities,
    ).get_energy_result().energy_usage.values[0] == pytest.approx(3.52044536)
