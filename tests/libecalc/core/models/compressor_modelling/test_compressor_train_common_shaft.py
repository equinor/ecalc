...
class TestCompressorTrainCommonShaft:
    def test_evaluate_rate_ps_pd_downstream_choke_pressure_control(
        self, single_speed_compressor_train_common_shaft, fluid_model_medium
    ):
    ...
    def test_evaluate_rate_ps_pd_asv_rate_control(self, single_speed_compressor_train_common_shaft, fluid_model_medium):
    ...
    def test_evaluate_rate_ps_pd_common_asv(self, single_speed_compressor_train_common_shaft, fluid_model_medium):
...
class TestCalculateSingleSpeedCompressorStage:
    def test_rate_below_minimum_chart_rate(
        self, single_speed_compressor_train_stage, fluid_model_medium, fluid_service
    ):
        stage = single_speed_compressor_train_stage(shaft=SingleSpeedShaft())
        ...
        speed = stage.compressor.compressor_chart.curves[0].speed
        stage.compressor.shaft.set_speed(speed)
    ...
    def test_rate_within_chart_curve_range(
        self, single_speed_compressor_train_stage, fluid_model_medium, fluid_service
    ):
        stage = single_speed_compressor_train_stage(shaft=SingleSpeedShaft())
...
def test_calculate_single_speed_train(single_speed_compressor_train_common_shaft, fluid_model_medium, fluid_service):
    mass_rate_kg_per_hour = 200000.0
    inlet_pressure_train_bara = 80.0
    ...
    compressor_train.set_evaluation_input(
        fluid_model=fluid_model_medium, rate=1, suction_pressure=1, discharge_pressure=2
    )
...
def test_calculate_evaluate_rate_ps_pd_single_speed_train_with_max_rate(
    single_speed_compressor_train_common_shaft, fluid_model_medium
):
    rate = [300000]
    suction_pressure = [10]
    ...
    assert result.max_standard_rate[0] == pytest.approx(407354, rel=0.002)


def test_calculate_single_speed_train_zero_mass_rate(single_speed_compressor_train_common_shaft, fluid_model_medium):
...
def test_calculate_single_speed_compressor_stage_given_target_discharge_pressure(
    ...
    compressor_train.set_evaluation_input(
        fluid_model=fluid_model_medium, rate=1, suction_pressure=1, discharge_pressure=2
    )  # dummy values for rates and pressures, only need fluid model

    stage: CompressorTrainStage = compressor_train.stages[0]
    inlet_stream_stage = fluid_service.create_stream_from_mass_rate(
        fluid_model=fluid_model_medium,
        pressure_bara=inlet_pressure_train_bara,
        temperature_kelvin=stage.inlet_temperature_kelvin,
        mass_rate_kg_per_h=mass_rate_kg_per_hour,
...
def test_points_above_and_below_maximum_power(single_speed_compressor_train_common_shaft, fluid_model_medium):
    compressor_train = single_speed_compressor_train_common_shaft(
...
class TestCompressorTrainCommonShaftOneRateTwoPressures:
    def test_single_point_within_capacity_one_compressor(self, variable_speed_compressor_train, fluid_model_medium):
    ...
    def test_points_above_and_below_maximum_power(self, variable_speed_compressor_train, fluid_model_medium):
    ...
    def test_single_point_rate_too_high_no_pressure_control(self, variable_speed_compressor_train, fluid_model_medium):
...
def test_find_and_calculate_for_compressor_shaft_speed_given_rate_ps_pd_invalid_point(
    ...
    standard_rate = fluid_service.mass_rate_to_standard_rate(fluid_model_medium, mass_rate_kg_per_hour)
    # rate too large
...
def test_get_max_standard_rate_with_and_without_maximum_power(variable_speed_compressor_train, fluid_model_medium):
    ...
    compressor_train.set_evaluation_input(
        fluid_model=fluid_model_medium, rate=1, suction_pressure=30, discharge_pressure=100
    )
