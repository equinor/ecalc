...
class TestSingleSpeedCompressorTrainCommonShaft:
    def test_evaluate_rate_ps_pd_downstream_choke_pressure_control(
        ...
        assert list(result.outlet_stream.pressure_before_choking) == pytest.approx(
            [304.9484, 304.9484, 367.5457, 216.2681]
        )
        assert result.inlet_stream.pressure == pytest.approx(suction_pressures)
        assert result.power == pytest.approx([14.54513, 14.54513, 16.05275, 14.68648])
    ...
    def test_evaluate_rate_ps_pd_asv_rate_control(self, single_speed_compressor_train_common_shaft_asv_rate_control):
        ...
        np.testing.assert_almost_equal(
            result.outlet_stream.pressure,
            [166.95, 300.00, 304.95, 300.00, 219.73],
            decimal=2,
        )
        np.testing.assert_almost_equal(
            result.outlet_stream.pressure_before_choking,
...
class TestCalculateSingleSpeedCompressorStage:
    def test_rate_below_minimum_chart_rate(self, single_speed_compressor_train_stage, medium_fluid):
        ...
        assert result.outlet_stream.pressure_bara == pytest.approx(173.36907174931252)
        assert result.outlet_stream.temperature_kelvin == pytest.approx(374.7067451568773)
        assert result.outlet_stream.density_kg_per_m3 == pytest.approx(117.13761191999643)
    ...
    def test_rate_within_chart_curve_range(self, single_speed_compressor_train_stage, medium_fluid):
        ...
        assert result.outlet_stream.pressure_bara == pytest.approx(140.1094987210056)
        assert result.outlet_stream.temperature_kelvin == pytest.approx(355.6816851340088)
        assert result.outlet_stream.density_kg_per_m3 == pytest.approx(103.15063443870191)
...
def test_calculate_single_speed_train(single_speed_compressor_train):
    ...
    # Stability tests
    assert result.power_megawatt == pytest.approx(14.514833589294827)
    assert result.discharge_pressure == pytest.approx(304.10486722419785)
    assert result.stage_results[0].outlet_stream.pressure_bara == pytest.approx(140.1094987210056)
    assert result.stage_results[1].inlet_stream.pressure_bara == pytest.approx(140.1094987210056)
...
def test_calculate_single_speed_train_zero_mass_rate(medium_fluid, single_speed_compressor_train):
    ...
    assert result.energy_usage == pytest.approx([0.0, 0.14898322782599177, 0.14898322782599177], rel=10e-5)
