...
from libecalc.infrastructure.file_io import EcalcFile
...
def read_resource_from_filepath(resource_path: Path) -> MemoryResource:
    ...
    if EcalcFile.is_csv(resource_path):
        return MemoryResource.from_path(resource_path)
...
def _read_resources(directory: Path, resource_names: list[str]) -> dict[str, MemoryResource]:
    ...
    #     resources[resource_name] = MemoryResource.from_path(directory / resource_name)
    #     # resources[resource_name] = read_resource_from_filepath(directory / resource_name)
    # return resources
    return {name: MemoryResource.from_path(directory / name) for name in resource_names}
