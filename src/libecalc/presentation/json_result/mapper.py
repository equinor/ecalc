        ...
        if consumer_node_info.component_type in [ComponentType.COMPRESSOR, ComponentType.COMPRESSOR_SYSTEM]:
            ...
            for model in consumer_result.models:
                ...
                inlet_stream_condition = CompressorStreamConditionResult(
                    ...
                    actual_rate_before_asv_m3_per_hr=TimeSeriesFloat(
                        timesteps=model.timesteps,
                        values=model.inlet_stream_condition.actual_rate_before_asv_m3_per_hr
                        if model.inlet_stream_condition.actual_rate_before_asv_m3_per_hr is not None
                        else [math.nan] * len(model.timesteps),
                        unit=Unit.ACTUAL_VOLUMETRIC_M3_PER_HOUR,
                    ),
                    standard_rate_sm3_per_day=TimeSeriesRate(
                    ...
                    standard_rate_before_asv_sm3_per_day=TimeSeriesRate(
                        timesteps=model.timesteps,
                        values=model.inlet_stream_condition.standard_rate_before_asv_sm3_per_day
                        if model.inlet_stream_condition.standard_rate_before_asv_sm3_per_day is not None
                        else [math.nan] * len(model.timesteps),
                        unit=Unit.STANDARD_CUBIC_METER_PER_DAY,
                        rate_type=RateType.STREAM_DAY,
                        regularity=regularity.for_timesteps(model.timesteps).values,
                ...
                outlet_stream_condition = CompressorStreamConditionResult(
                    ...
                    actual_rate_before_asv_m3_per_hr=TimeSeriesFloat(
                        timesteps=model.timesteps,
                        values=model.outlet_stream_condition.actual_rate_before_asv_m3_per_hr
                        if model.outlet_stream_condition.actual_rate_before_asv_m3_per_hr is not None
                        else [math.nan] * len(model.timesteps),
                        unit=Unit.ACTUAL_VOLUMETRIC_M3_PER_HOUR,
                    ),
                    standard_rate_sm3_per_day=TimeSeriesRate(
                    ...
                    standard_rate_before_asv_sm3_per_day=TimeSeriesRate(
                        timesteps=model.timesteps,
                        values=model.outlet_stream_condition.standard_rate_before_asv_sm3_per_day
                        if model.outlet_stream_condition.standard_rate_before_asv_sm3_per_day is not None
                        else [math.nan] * len(model.timesteps),
                        unit=Unit.STANDARD_CUBIC_METER_PER_DAY,
                        rate_type=RateType.STREAM_DAY,
                        regularity=regularity.for_timesteps(model.timesteps).values,
