...
from libecalc.presentation.yaml.domain.expression_time_series_unit_power_adjustment import (
    ExpressionTimeSeriesUnitPowerAdjustmentConstant,
    ExpressionTimeSeriesUnitPowerAdjustmentFactor,
)
...
def _get_unit_power_adjustment_values(
    unit_power_adjustment_reference: YamlStageUnitPowerAdjustment, expression_evaluator: ExpressionEvaluator
) -> tuple[list[float] | None, list[float] | None]:
    if unit_power_adjustment_reference is None:
        unit_power_adjustment_factor = None
        unit_power_adjustment_constant = None
    else:
        unit_power_adjustment_factor = unit_power_adjustment_reference.factor
        unit_power_adjustment_constant = unit_power_adjustment_reference.constant

    factor_values = ExpressionTimeSeriesUnitPowerAdjustmentFactor(
        time_series_expression=TimeSeriesExpression(
            expression=unit_power_adjustment_factor, expression_evaluator=expression_evaluator
        )
    ).get_values()
    constant_values = ExpressionTimeSeriesUnitPowerAdjustmentConstant(
        time_series_expression=TimeSeriesExpression(
            expression=unit_power_adjustment_constant, expression_evaluator=expression_evaluator
        )
    ).get_values()
...
class CompressorModelMapper:
        ...
        if not isinstance(train_spec, YamlUnknownCompressorStages):
            ...
            stages = [
                _create_compressor_train_stage(
                    ...
                    unit_power_adjustment_factor=_get_unit_power_adjustment_values(
                        unit_power_adjustment_reference=stage.unit_power_adjustment,
                        expression_evaluator=expression_evaluator,
                    )[0],
                    unit_power_adjustment_constant=_get_unit_power_adjustment_values(
                        unit_power_adjustment_reference=stage.unit_power_adjustment,
                        expression_evaluator=expression_evaluator,
                    )[1],
                )
    ...
    def _create_variable_speed_compressor_train(
        ...
        for stage in stages_data:
            ...
            unit_power_adjustment_factor, unit_power_adjustment_constant = _get_unit_power_adjustment_values(
                unit_power_adjustment_reference=stage.unit_power_adjustment, expression_evaluator=expression_evaluator
            )
    ...
    def _create_variable_speed_compressor_train_multiple_streams_and_pressures(
        ...
        for stage_index, stage_config in enumerate(model.stages):
            ...
            unit_power_adjustment_factor, unit_power_adjustment_constant = _get_unit_power_adjustment_values(
                unit_power_adjustment_reference=stage_config.unit_power_adjustment,
                expression_evaluator=expression_evaluator,
            )
