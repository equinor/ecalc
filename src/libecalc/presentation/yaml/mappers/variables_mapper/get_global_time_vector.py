...
def get_global_time_vector_no_frequency(
...
def get_global_time_vector(
    ...
    """
    Generate a sorted list of unique datetime objects representing the global time vector,
    including additional dates at the specified frequency if provided.
    ...
    Args:
        ...
        additional_dates (set[datetime] | None): Optional set of additional dates to include.
        frequency (libecalc.common.time_utils.Frequency): Frequency for generating additional dates. If Frequency.NONE, no extra dates are added.

    Returns:
        list[datetime]: Sorted list of datetime objects within the specified boundaries, including frequency-based dates if specified.

    Raises:
        ValidationError: If neither a time vector nor both start and end are provided.
    """
    time_vector = set(
        get_global_time_vector_no_frequency(
