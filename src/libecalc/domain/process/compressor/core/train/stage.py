...
from libecalc.domain.process.entities.process_units.compressor.compressor import Compressor, CompressorUndefined
...
class CompressorTrainStageNew:
    ...
    def add_recirculation_rate(
        ...
        Returns:
            FluidStream: Updated inlet stream.
        """
        if (asv_rate_fraction is not None and asv_rate_fraction > 0) and (
            asv_additional_mass_rate is not None and asv_additional_mass_rate > 0
        ):
            raise IllegalStateException("asv_rate_fraction and asv_additional_mass_rate cannot both be > 0")
        if asv_rate_fraction is not None and not (0.0 <= asv_rate_fraction <= 1.0):
            raise IllegalStateException("asv_rate_fraction must be in [0.0, 1.0]")
    ...
    def evaluate(
        ...
        if not (
            ...
            msg = (
                ...
                f"({self.compressor.compressor_chart.minimum_speed}-{self.compressor.compressor_chart.maximum_speed})."
            )
            logger.exception(msg)
            raise IllegalStateException(msg)
...
class UndefinedCompressorStageNew(CompressorTrainStageNew):
    """A stage without a defined compressor chart is 'undefined'.
    ...
    def __init__(
        self,
        compressor: CompressorUndefined,
        temperature_setter: TemperatureSetter,
        liquid_remover: LiquidRemover | None,
        rate_modifier: RateModifier,
        pressure_modifier: DifferentialPressureModifier | None = None,
    ):
        self.validate_predefined_chart(compressor.compressor_chart, compressor.polytropic_efficiency)
        self.validate_polytropic_efficiency(compressor.polytropic_efficiency)
        super().__init__(
            compressor=compressor,
        ...
        self.polytropic_efficiency = compressor.polytropic_efficiency
