...
#         id: str,
#         name: str,
#         temporal_generator_set_model: TemporalModel[GeneratorSetProcessUnit],
#     ):
...
#         # Delegate evaluation to evaluate_with_periods
#         fuel_rate, power_capacity_margin = self.evaluate_with_periods(
#             generator_set_model=dict(self.temporal_generator_set_model.items()),
#             power_requirement=np.asarray(power_requirement.values),
...
#
#     @staticmethod
#     def evaluate_fuel_and_margin_across_periods(
#             generator_set_model: dict[Period, GeneratorSetProcessUnit],
#             power_requirement: NDArray[np.float64],
#             periods: Periods,
#             actual_period: Period,
#     ) -> tuple[NDArray[np.float64], NDArray[np.float64]]:
#         """Evaluate fuel rate and power capacity margin across all process units."""
#         result_fuel_rate = np.full_like(power_requirement, fill_value=np.nan).astype(float)
#         result_power_margin = np.zeros_like(power_requirement).astype(float)
#
#         for period, process_unit in generator_set_model.items():
#             if Period.intersects(period, actual_period):
#                 fuel_rate, power_margin = process_unit.evaluate_fuel_and_margin_for_period(
#                     power_requirement, periods, period
#                 )
#                 start_index, end_index = period.get_period_indices(periods)
#                 result_fuel_rate[start_index:end_index] = fuel_rate
#                 result_power_margin[start_index:end_index] = power_margin
#
#         return result_fuel_rate, result_power_margin
#
