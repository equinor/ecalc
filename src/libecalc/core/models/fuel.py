...
class FuelModel:
    ...
    def evaluate_emissions(
        self, variables_map: VariablesMapService, fuel_rate: NDArray[np.float64]
        ...
        for period, model in self.temporal_fuel_model.items():
            if Period.intersects(period, variables_map.get_period()):
                start_index, end_index = period.get_timestep_indices(variables_map.get_time_vector())
