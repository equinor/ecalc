...
def propagate_stream_many(
    process_units: Sequence[ProcessUnit], inlet_stream: FluidStream
) -> FluidStream | PropagationFailure:
    current_stream: FluidStream = inlet_stream
    for process_unit in process_units:
        next_stream = process_unit.propagate_stream(current_stream)
        if isinstance(next_stream, PropagationFailure):
            return next_stream
...
class ProcessPipelineRunner(ProcessRunner):
    ...
    def _propagate_stream_to_id(
        ...
        to_id: ProcessUnitId,
    ) -> tuple[FluidStream | PropagationFailure, bool]:
        current_stream: FluidStream = inlet_stream
        for unit in units:
            if unit.get_id() == to_id:
                return current_stream, True
            next_stream = unit.propagate_stream(inlet_stream=current_stream)
            if isinstance(next_stream, PropagationFailure):
                return next_stream, False
            current_stream = next_stream
