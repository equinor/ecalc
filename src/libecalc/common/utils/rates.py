...
class TimeSeries(BaseModel, Generic[TimeSeriesValue], ABC):
    ...
    def reindex_periods(
        ...
        """
        new_values: DefaultDict[str, Union[float, str]] = defaultdict(float)
        new_values.update({str(t): fillna for t in new_periods})
        for t, v in zip(self.periods, self.values):
            if str(t) in new_values:
                new_values[str(t)] = v
            else:
                logger.warning(
...
class TimeSeriesBoolean(TimeSeries[bool]):
    def resample(self, freq: Frequency, include_start_date: bool = True, include_end_date: bool = True) -> Self:
        ...
        for start_period, end_period in zip(new_time_steps[:-1], new_time_steps[1:]):
            start_index = self.all_dates().index(max([date for date in self.all_dates() if date <= start_period]))
            end_index = self.all_dates().index(max([date for date in self.all_dates() if date < end_period]))
...
class TimeSeriesVolumesCumulative(TimeSeries[float]):
    ...
    def resample(
        ...
        if ds.index[-1] not in new_time_steps:
            logger.warning(
    ...
    def __truediv__(self, other: object) -> TimeSeriesIntensity:
...
class TimeSeriesIntensity(TimeSeries[float]):
    """Also keeping track of the original cumulative volumes for potential resampling of intensities"""

    dividend: TimeSeriesVolumesCumulative
    divisor: TimeSeriesVolumesCumulative
    ...
    def resample(
        ...
        """
        TODO: When resampling intensities, should it not be based on first resampling the underlying emissions and
            hcexport and then recalculating the intensities. Intermediate solution implemented where the intensity
            is assigned to the middle_date in the period, and linear interpolation is used between the values.
        ...
        resampled_dividend = self.dividend.resample(freq, include_start_date, include_end_date)
        resampled_divisor = self.divisor.resample(freq, include_start_date, include_end_date)

        return TimeSeriesIntensity(
            periods=resampled_dividend.periods,
            values=np.divide(
                resampled_dividend.values,
                resampled_divisor.values,
                out=np.full_like(resampled_dividend.values, fill_value=np.nan),
                where=np.asarray(resampled_divisor.values) != 0.0,
            ).tolist(),
            unit=self.unit,
            dividend=resampled_dividend,
            divisor=resampled_divisor,
        )
